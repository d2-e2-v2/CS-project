...
def differential_evolution(
    ...
    max_generations : maximum iterations (default 100, matches Fig 6b in paper)
    ...
    -------
    DEResult
        convergence_history contains the population-best fitness recorded
        *once per generation* (length = actual generations run + 1 for the
        initial evaluation), so the DE convergence plot always spans the
        full 0 → max_generations x-axis regardless of early stopping.
    """
    ...
    # Record population-best at generation 0 (initial population)
    history = [best_fit]
    ...
    for gen in range(max_generations):
        ...
        # ── Record population-best once per generation ────────────────────
        # This guarantees convergence_history[gen+1] = best at end of gen
        ...
        if gen > 5 and abs(history[-1] - history[-6]) < tol:
            ...
            # Pad remaining generations with the converged value so the plot
            # always fills 0 → max_generations on the x-axis.
            remaining = max_generations - (gen + 1)
            history.extend([best_fit] * remaining)
            break
